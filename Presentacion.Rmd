...
```{r, fig.width = 3, fig.height = 3, warning=FALSE, echo=FALSE, message = FALSE}
n <- 1
...
h_2(X) = h(x_{i,j} - X_j)
$$
```{r, fig.width = 3, fig.height = 2, warning=FALSE, echo=FALSE, message = FALSE}
x_left <- seq(-5, -3, 1)
x_right <- seq(-3, -1, 1)
y_left <- -(x_left + 3)
y_right <- 3 + x_right
...
ejemplo_4 |>
  ...
  # ggtitle(paste("MARS con",n,"nodo")) + 
  annotate("text", x = -1.9, y = 1.9, label = TeX(r'($h_{1} = (X_j-x_{ij})_+$)'), size = 3) +
  annotate("text", x = -4.1, y = 1.9, label = TeX(r'($h_{2} = (x_{ij}-X_j)_+$)'), size = 3) +
  ylab("y") + xlab("x")+
  theme_classic()
```
...
El modelo que utilizamos entonces para juntar todas las variables es uno aditivo en $\boldsymbol\beta$: 
$$
...
* Este proceso se repite para cada uno de los pares reflejados restantes, resolviendo OLS (ordinary least squares) para determinar el nuevo $\boldsymbol\beta$, y agregando aquel que continue minimizando el error. 
...
resultados_tabla <- res_mars |> 
  ...
  rename(resultados_tabla, MARS = estimate_mars, Arboles = estimate_arboles, Bosques = estimate_bosques)
```
